...
# Initialize confirm flag for clearing (safe)
...
# Note: The above header keeps the top broom you had. The in-input-area broom (left) is added below.
...
# --- Chat input area: remove the rocket send button, keep a broom erase on the left ---
col1, col2 = st.columns([8, 1.2])

with col1:
    # Keep the same key "chat_input" that your existing code expects.
    user_input = st.text_input("Type your message...", key="chat_input", placeholder="Ask me anything...")

with col2:
    # Remove the rocket send button per your instruction.
    # Add a broom erase button (left-side in the input area style you asked).
    erase = st.button("🧹")

# Handle erase confirmation flow (minimal changes)
if erase:
    st.session_state.confirm_clear = True

if st.session_state.confirm_clear:
    st.markdown("### Are you sure you want to clear the chat history?")
    c1, c2 = st.columns([1, 1])
    with c1:
        if st.button("Yes — clear history"):
            # Clear all chat-related session keys safely
            for k in ["messages", "chat_history", "history", "readme_preview", "readme_full"]:
                if k in st.session_state:
                    st.session_state[k] = [] if isinstance(st.session_state.get(k), list) else None
            # Reset greet and confirm flags to let greeting show again if you like
            st.session_state.greeted = False
            st.session_state.confirm_clear = False
...
# -----------------------
# Send question -> Groq
# (Enter / text_input flow remains the same — we did NOT add any send button logic)
# -----------------------
if user_input:
    # append user message to history (this is the flow you already had)
